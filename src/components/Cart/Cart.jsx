...
const Cart = () => {
    const { cartList, removeItem, cartListTotal, setCartListTotal } = useCartContext()
    ...
    return (
        <div>
            ...
            {cartList.length > 0 ? cartList.map(element => {
                return (
                    <div key={element.item.id} className=" d-flex justify-content-center card">
                        <div className="row">
                            <div className="col-1">
                                <button className="btn btn-sm btn-danger" onClick={() => { deleteItem(element) }}>x</button>
                            </div>
                            <div className="col-2">
                                {element.quantity}
                            </div>
                ...
                <Link to={`/`}>
                    <button className="btn btn-outline-primary btn-block">Start Shopping</button>
                </Link>
            </div>
            }
            {cartList.length > 0 && <div className=" d-flex justify-content-center card">
                <div key="checkoutHeader" className="row">
                    <div className="col-1"></div>
                    <div className="col-2">
